...
* **Infraestructura y Almacenamiento:** Implementación de **IDM (Identity Management)** en arquitectura Maestro/Réplica. Gestión de almacenamiento optimizado con **VDO (Virtual Data Optimizer)** y LVM.
...
Infraestructura Crítica y Almacenamiento
========================================

**Entorno de Virtualización con Optimización VDO**:

* **Descripción:** Implementación de un sistema de almacenamiento eficiente para entornos `libvirt`. Utilizo **VDO** sobre LVM para proporcionar deduplicación y compresión de datos en tiempo real, permitiendo un aprovisionamiento delgado (*thin provisioning*) de alta densidad.
* **Detalle Técnico (Pool de Almacenamiento):**
    * Configuración de un volumen lógico indexado (VDO) que expande la capacidad física (NVMe) a un almacenamiento virtual.
    * Punto de montaje optimizado en ``/var/lib/virt_storage`` para el despliegue masivo de máquinas virtuales.
* **Tecnologías:** RHEL/CentOS, VDO, LVM, Libvirt, NVMe.

**Gestión de Identidad (IDM/FreeIPA)**:

* **Descripción:** Despliegue de una infraestructura de gestión de identidades basada en arquitectura **Maestro-Réplica**.
* **Funcionalidades:** Centralización de autenticación (Kerberos), políticas de acceso (LDAP), DNS y gestión de certificados (PKI) para asegurar la consistencia y alta disponibilidad de los servicios en el nodo de desarrollo.
...
* **Entorno de colaborativo**: `raulDev-heliogabalo.io`
...
* **Web Profesional y Portafolio**: `raulvilchez.org`
